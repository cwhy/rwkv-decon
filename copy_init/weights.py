...
class WeightConfig(Protocol):
    def init(self, rng_key: SafeKey) -> dict[str, Arr]:
        ...
...
def get_normal_weights_config_(path: Path, model_name: str,
                               non_normal_weight_tags: Optional[dict[str, WeightConfigType]]=None) -> dict[str, NormalWeight]:
    try:
        return load_weight_configs_(path, model_name)
    except FileNotFoundError:
        weight_infos: dict[str, NormalWeight] = {}
...
def load_weight_configs_(path: Path, model_name: str) -> dict[str, NormalWeight]:
...
def init(w_infos: dict[str, NormalWeight], rng_key: SafeKey) -> dict[str, Arr]:
